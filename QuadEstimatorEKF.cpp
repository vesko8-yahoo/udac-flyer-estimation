...
*/
    Quaternion<float> quatern = Quaternion<float>().FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
    quatern.IntegrateBodyRate(gyro, dtIMU);
    float predictedPitch = quatern.Pitch();
    float predictedRoll = quatern.Roll();
    ekfState(6) = quatern.Yaw();
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    predictedState(3) = predictedState(3) + accel.x*dt;
    predictedState(4) = predictedState(4) + accel.y*dt;
    predictedState(5) = predictedState(5) + (accel.z-CONST_GRAVITY)*dt;
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    if (magYaw - zFromX(0) > F_PI) zFromX(0) += 2.f*F_PI;
    if (magYaw - zFromX(0) < -F_PI) zFromX(0) -= 2.f*F_PI;
