...
I followed the steps above and I came up with `MeasuredStdDev_GPSPosXY = 0.702` and `MeasuredStdDev_AccelXY = 0.499`. Those values worked well.
...
![attitude example](images/scenario7.png)

I used the suggestion and I used the Quaternion class, which has a handy FromEuler123_RPY function for creating a quaternion from Euler Roll/PitchYaw and the IntegrateBodyRate function.
...
I implemented `PredictState()` using `dt`. I also implemented the calculation of the rotation matrix in `GetRbgPrime()` per the suggestion.
And finally I implemented `Predict()`. I had to tune the `QPosXYStd` and the `QVelXYStd` process parameters and ended up with `QPosXYStd = .10` and `QVelXYStd = .30`.
...
I implemented the UpdateFromMag() per the instructions and after tuning `QYawStd` I ended up with `QYawStd = .30`.
...
After adding `UpdateFromGPS()` I had to tune the GPS parameter to the following like this: `GPSVelXYStd = .10` and `GPSVelZStd = .27`. 
...
After following the steps above it was failing scenario 11. To get it to pass scenario 11 I had to change some parameters inside `QuadControlParams.txt`. I ended up changing `kpVelXY` from 15 to 13, and `kpVelZ` from 18 to 15. That was enough for it to pass the scenario.
